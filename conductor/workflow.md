...
3. **Branch-Based Development:** NEVER commit directly to the `main` branch. Create a feature branch for every task or phase, and merge via Pull Request (PR).
4. **Test-Driven Development:** Write unit tests before implementing functionality
5. **High Code Coverage:** Aim for >0% code coverage for all modules
...
2. **Create Feature Branch:** Before beginning work, create and switch to a new feature branch from `main`. Use the format `feature/<task-name-or-id>`.
   ```bash
...
2. After PR approval and passing CI checks, merge to `main`.
